...
from prefect import State, flow, get_client, task
...
async def find_long_running_flows() -> list[UUID]:
        ...
        flow_runs = await client.read_flow_runs(
            flow_run_filter=FlowRunFilter(
                ...
                start_time=FlowRunFilterStartTime(
                    before_=datetime.now(timezone.utc) - timedelta(hours=threshold_hours),
                ),
            )
        )
    ...
    for flow_run in flow_runs:
        if runtime.flow_run.id != flow_run.flow_id:
            ...
            logger.info("El ID es el del Watchdog actual. No se cancelara. ID: %s", flow_run.flow_id)
        ...
        flow_runs = await client.read_flow_runs(
            flow_run_filter=FlowRunFilter(
                ...
                expected_start_time=FlowRunFilterExpectedStartTime(
                    before_=datetime.now(timezone.utc) - timedelta(hours=threshhold_hours),
                ),
            )
        )
...
async def cancel_flow_runs(flow_run_id):
    ...
    async with get_client() as client:
        ...
        state = State(type=StateType.CANCELLED, message="Cancelado por watchdog")
...
async def watchdog():
    ...
    time_difference = datetime.now(flow_timezone) - runtime.flow_run.scheduled_start_time
